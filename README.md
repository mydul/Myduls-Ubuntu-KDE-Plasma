# Myduls-Ubuntu-KDE-Plasma
A Linux shell script to build the Ubuntu KDE Plasma desktop environment from minimal installation. This is a personal script for Kubuntu 22.04.2 LTS installation.

## Table of contents

- 🚀 [Project Goal](#Project-Goal)
- 📚 [Content](#Content)
- 🗄 [Instruction for installation](#Instruction-for-installation)
- ⚖  [License](#License)
- 💬 [Acknowledgements](#Acknowledgements)


## Project Goal

This project's goal is to learn and share Linux shell scripting. Mainly, the project is **"Linux shell script (Bash) to build the Ubuntu KDE Plasma desktop environment from the minimal installation"**.

## Content

- Introduction: Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Donec odio. Quisque volutpat mattis eros. Nullam malesuada erat ut turpis. Suspendisse urna nibh, viverra non, semper suscipit, posuere a, pede.

- Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Donec odio. Quisque volutpat mattis eros. Nullam malesuada erat ut turpis. Suspendisse urna nibh, viverra non, semper suscipit, posuere a, pede.

- Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Donec odio. Quisque volutpat mattis eros. Nullam malesuada erat ut turpis. Suspendisse urna nibh, viverra non, semper suscipit, posuere a, pede.
- Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Donec odio. Quisque volutpat mattis eros. Nullam malesuada erat ut turpis. Suspendisse urna nibh, viverra non, semper suscipit, posuere a, pede.

## Instruction for installation

Source Code:

```python
# This program prints Hello, world!

print('Hello, world!')
```

Output:

> Hello, world!


## License

Distributed for all newbies by [personal](https://github.com/mydul) licensing under the MIT license.


## Acknowledgements

- [x]  [Mohammad Shahi Amran Hossain](https://github.com/)


## Authors

<img align='right' src="https://camo.githubusercontent." width="230">

### Hi, I'm Mydul Islam! <img src="https://media.giphy.com/media/fYSnHlufseco8Fh93Z/giphy.gif" width="30">
[![GitHub Mydul](https://img.shields.io/github/followers/mydul?label=follow&style=social)](https://github.com/mydul)

<p><em>I am a masters student at <a href="http://www.uni-siegen.de"> Universität Siegen. </a>
  I love to have fun with programming, scripting, and Linux. <br>
</em></p>  

A little more about me...

```javascript
const mydul = {
  pronouns: ["He" "|" "Him"],
  code: ["C", "C++", "Python", "HTML", "CSS", "LaTeX", "MATLAB", "VHDL", "Arduino" , "Shell script"],
  tools: ["Microsoft Office", "Proteus", "PSpice", "LTSpice", "OriginPro", "MicroWind", "COMSOL", "Cadence"],
  accustomed: ["Linux", "Windows", "Raspbian"],
  experience: {
                        research_assistant: "Universität Siegen",
                        delegate: "Model United Nations",
                        member: "IEEE"
                      },
 challenge: "I am seeking a full-time position in the field of Nanotechnology, and Quantum Electronics"
}
```
---
⭐️[@mydul](https://github.com/mydul)
